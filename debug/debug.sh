...
exec-before-load = """bash -o pipefail -c '\
  ...
  | /opt/rocm/opencl/bin/x86_64/clang -x assembler -target amdgcn--amdhsa -mcpu=`/opt/rocm/bin/rocminfo | grep -om1 gfx9..` \
    -mno-code-object-v3 -Igfx9/include -o tmp/debug.co -'"""
EOF
